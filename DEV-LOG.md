...
| 文件 | 纠正点 |
|------|--------|
| `ROADMAP.md` | X4 参数级补全、X5 本地脚本包分享改为代码已落地；区分在线市场远期；写入 v1.4.x 收口与 A8 partial；记录世界书导入回滚 |
| `DEVELOPMENT-PLAN.md` | 文首状态、进度总览、里程碑与 A6 表述对齐 HEAD `73436cc` / DB v34；去掉“X4 参数补全与 X5 未启动”过时句 |

### A6 决策（世界书 automation）

| 项 | 结论 |
|----|------|
| QR `chat_open` / `assistant_reply` | 保持已实现，不回退 |
| `world_book_entries.automation_id` | **保留 DB 字段**（兼容 ST 导入/未来扩展） |
| 命中世界书 entry 后自动触发同名 QR | **明确不做** |
| 理由 | 生成热路径自动跑脚本：安全边界复杂（权限/auto-run）、难预期、与“优化优先”冲突；当前无 UI 配置闭环与测试设计 |
| 验收状态 | 决策已文档化 = 本项关闭；运行时接线 **不做**，故不标功能通过 |

### 验证

- 文档 diff 人工审阅；无业务代码改动。
- 本批不跑 Gradle。

**后续**: 当前 HEAD 全量门禁复跑（A8）→ 可自动测试加强 → X5/主链路打磨。
